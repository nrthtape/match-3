...
export class Game extends Scene {
        ...
        tile.moveTo(selectedTile.field.position, 0.2).then(() => {
            ...
            if (!reverse) {
                ...
                if (matches.length) {
                    ...
                    // if there are no combinations after the main swap,
                    // then perform a reverse swap by running the same method, but with the reverse parameter
    ...
    addTiles() {
        return new Promise(resolve => {
            const emptyFields = this.calculateEmptyFields();

            const rows = this.board.rows;
            const cols = this.board.cols;

            const total = emptyFields.fields;
            const emptyRows = emptyFields.rows;
            ...
            for (let row = 0; row < rows; row++) {
                for (let col = 0; col < cols; col++) {
                    ...
                    if (field.tile) continue; // skip non-empty fields
                    // create a new tile
                    const tile = this.board.createTile(field);
                    // put it above the board
                    tile.sprite.y = -tile.sprite.height * 3; // calculate y position based on row number
                    const delay = 0.05 * (emptyRows - row - 1 + (col)); // calculate delay based on row and column numbers
    ...
    calculateEmptyFields() {
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                if (!emptyRows.includes(row)) emptyRows.push(row);
                if (!emptyCols.includes(col)) emptyCols.push(col);

                emptyFields.push(field);
            }
        }

        return {fields: emptyFields.length, rows: emptyRows.length, cols: emptyCols.length}
    }
    ...
    removeMatches(matches) {
        ...
        matches.forEach((match, i) => {
            match.forEach((tile, j) => {
                let delay = 0.05 * j + i * 0.1 * matches.length;
            ...
            if (this.gameStart) {
                if (match.length === 3) console.log('3');
                if (match.length === 4) console.log('4');
                if (match.length === 5) console.log('5');
            }
        });
    ...
    createBackground() {
        this.bg = App.solid(0x111111);
        // this.bg = App.sprite("bg");
        this.bg.width = App.config.size[0];
        this.bg.height = App.config.size[1];
